...
select distinct games, 
       count(distinct sport) as num_of_sports
from athlete_events
group by games
...
where age=(select max(age) 
                  from athletes..athlete_events WHERE medal = 'gold') and 
                  medal='gold'


--10. Find the Ratio of male and female athletes participated in all olympic games.

select concat('1 : ',round(convert(float, 
...
WHERE g.gold = (SELECT MAX(gold) FROM cte_gold WHERE games = g.games) AND
s.silver = (SELECT MAX(silver) FROM cte_silver WHERE games = s.games) AND 
b.bronze = (SELECT MAX(bronze) FROM cte_bronze WHERE games = b.games)
ORDER BY g.games;
...
WHERE g.gold = (SELECT MAX(gold) FROM cte_gold WHERE games = g.games) AND 
s.silver = (SELECT MAX(silver) FROM cte_silver WHERE games = s.games) AND 
b.bronze = (SELECT MAX(bronze) FROM cte_bronze WHERE games = b.games) AND 
t.total = (SELECT MAX(total) FROM cte_total WHERE games = t.games)
ORDER BY g.games;
